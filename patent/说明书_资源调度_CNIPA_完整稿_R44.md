...
当前附图（图1至图4）为实验阶段产物，用于技术方案沟通与审查答复准备。
在正式向 CNIPA 提交前，应执行以下图面规范化动作：
1. 将图面全部标注统一为中文术语，并与权利要求和说明书术语保持一致。
2. 为图1和图2补齐模块、步骤和数据流的参考编号。
3. 将图3和图4中的坐标轴、图例、注释统一中文化。
4. 使图面标号与附图标记说明表逐项一致，避免“图内标号-文字说明”不一致。
...
记录任务完成、失败、超时和强制移除事件。
记录模式切换事件。
记录阻断事件；每条阻断事件至少包括任务标识、阻断原因类别和阻断时间戳。
统计阻断事件总数和唯一阻断任务总数，并写入当轮审计汇总。
记录启动和停止事件。
当向目标任务发出终止请求后，若在预设终止超时时间内未完成终止，则执行强制移除，并同时记录终止失败事件与超时移除事件。
记录参数调优事件。
记录资源画像更新事件。
事件日志支持容量上限控制。
日志超上限时按先进先出策略截断。
...
所述调优参数至少包括准入阈值偏置量、调度窗口时长和任务负载参数。
所述任务负载参数至少包括任务数量、任务持续时长和任务基础内存占用量。
每轮调优结果写入审计轨迹。
